...
ggplot(data=ad_kr, aes(AD)) + geom_bar(fill = "#0073C2FF") + xlab("Korea's Count of Antidumping Measures") + ylab("Frequency (Number of Years)")
...
ggplot(data=ad_2014, aes(x=gdp, y=AD)) + geom_smooth() + ylab("Count of Antidumping Measures")
...
# 2-1. Motor Cars
data(mtcars)
ggplot(data=mtcars, aes(x=as.factor(cyl), y=disp)) + geom_boxplot(linetype = "dashed", outlier.fill=0) +
  stat_boxplot(aes(ymin = ..lower.., ymax = ..upper..), outlier.shape = 1) +
  stat_boxplot(geom = "errorbar", aes(ymin = ..ymax..)) +
  stat_boxplot(geom = "errorbar", aes(ymax = ..ymin..)) + xlab("Number of Cylinders") + ylab("Displacement") + ylim(0, 400)
...
# ggplot(data=VDemRed_Argen_Brazil, aes(x=year, y=v2x_polyarchy, col=as.factor(country_name))) + geom_point() + ylim(0, 0.8)
